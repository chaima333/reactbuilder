...
export const canDrop = (parentType: string, childType: string): boolean => {
  if (parentType === "root") {
    const childConfig = blockRegistry[childType];
    return childConfig?.rules?.allowedParents?.includes("root") ?? false;
  }

  const parentConfig = blockRegistry[parentType];
  const childConfig = blockRegistry[childType];
  ...
  if (parentType === "gridItem") {
    if (childType === "gridItem") return false;
    return parentConfig.allowedChildren?.includes(childType as any) ?? true;
  }

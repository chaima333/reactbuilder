...
const getDashboardStats = async (req, res) => {
    try {
        ...
        const [totalSites, totalPages, totalViews, recentActivities, storageUsed] = await Promise.all([
            models_1.Site.count({ where: { ownerId: userId, status: 'active' } }),
            models_1.Page.count({ where: { userId } }),
            models_1.Page.sum('views', { where: { userId } }),
            models_1.ActivityLog.findAll({
                where: { userId },
                limit: 10,
                order: [['createdAt', 'DESC']],
                include: [
                    { model: models_1.Site, as: 'site', attributes: ['name', 'subdomain'] }
                ]
            }),
...
const getActivityLog = async (req, res) => {
    try {
        ...
        if (siteId)
            where.siteId = siteId;
        const activities = await models_1.ActivityLog.findAndCountAll({
            where,
            limit: Number(limit),
            offset: Number(offset),
            order: [['createdAt', 'DESC']],
            include: [
                { model: models_1.User, as: 'user', attributes: ['id', 'name', 'email', 'avatar'] },
                { model: models_1.Site, as: 'site', attributes: ['id', 'name', 'subdomain'] }
            ]
        });
        res.json({
            success: true,

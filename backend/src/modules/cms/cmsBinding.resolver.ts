export type CmsFieldBindingDefinition = {
  key: string;
};
...
export const createCmsFieldKeySet = (
  fields: CmsFieldBindingDefinition[] = []
) =>
  new Set(
    fields
      .map((field) =>
...
const resolveCmsBindingsInternal = (
  value: unknown,
  entryData: Record<string, unknown>,
  fields: CmsFieldBindingDefinition[],
  options: CmsBindingResolverOptions,
...
export const resolveCmsBindingsInTree = <T>(
  ...
  return resolveCmsBindingsInternal(
    value,
    entryData,
    fields,
    options,
